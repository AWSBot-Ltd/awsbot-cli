...
def get_headers():
    ...
    if not token:
        console.print("[bold red]Error:[/bold red] GITHUB_TOKEN not set in this profile.")
...
def audit_repos(
        ...
        fix: bool = typer.Option(False, "--fix", help="Actually apply changes (Make private, delete forks)"),
):
    """
    ...
    console.print(f"Scanning [bold]{org}[/bold] (Mode: {'FIX' if fix else 'DRY RUN'})...")
        ...
        for repo in repos:
            ...
            if is_fork:
                if fix:
                    ...
                    requests.delete(f"https://api.github.com/repos/{org}/{name}", headers=headers)
                else:
            ...
            # Case 2: Enforce Private
                ...
                if fix:
                    ...
                    requests.patch(f"https://api.github.com/repos/{org}/{name}", json={"private": True},
                                   headers=headers)
                else:
                    console.print(f"[yellow]Found Public Repo (Dry Run):[/yellow] {name}")
...
def transfer_all(
        ...
        source_user: str = typer.Option(None, help="User to move FROM (defaults to authenticated user)"),
):
    """
